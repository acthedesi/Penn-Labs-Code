...
def get_clubs(soup):
    ...
    """
    soups_for_clubs = []
    club_elements = get_elements_with_class(soup, 'div', 'box');
    for club in club_elements: 
        club_soup = BeautifulSoup(str(club), "html.parser")
        soups_for_clubs.append(club_soup)
    return soups_for_clubs # TODO: Implement this function
...
def get_club_description(club):
    ...
    if len(elts) < 1:
        return ''
    return elts[0].text # TODO: Implement this function
...
def get_club_tags(club):
    ...
    if len(elts) < 1:
        return ''
    for elt in elts:
        club_tags.append(elt.text)
    return club_tags # TODO: Implement this function
